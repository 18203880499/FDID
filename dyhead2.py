...
__all__ = ['Detect_dyhead3','Seg_dyhead3']
...
class Seg_dyhead3(Detect_dyhead3):
    """YOLOv8 Segment head with Dynamic Head v3"""
    dynamic = False  # force grid reconstruction
    export = False  # export mode
    shape = None
    anchors = torch.empty(0)  # init
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=()):
        ...
        Args:
            ...
            npr (int): number of prototype features
            ch (tuple): input channels
        """
        super().__init__()
        self.nc = nc  # number of classes
        self.nm = nm  # number of masks
        self.npr = npr  # number of prototype features
        self.nl = len(ch)  # number of detection layers
        self.reg_max = 16  # DFL channels
        self.no = nc + self.reg_max * 4  # number of outputs per anchor for box/class
        self.stride = torch.zeros(self.nl)  # strides computed during build
        ...
        # Segmentation head
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, x, 3), nn.Conv2d(x, self.nm, 1)) for x in ch)

        # Prototype branch
        self.cv5 = nn.Sequential(
            Conv(ch[0], ch[0] // 4, 3),
            Conv(ch[0] // 4, ch[0] // 4, 3),
            nn.Conv2d(ch[0] // 4, self.npr, 1))
    ...
    def forward(self, x):
        """Process features through detection and segmentation heads."""
        # Apply Dynamic Head v3
        tensor_dict = {i: tensor for i, tensor in enumerate(x)}
        x = self.dyhead_tower(tensor_dict)
        x = list(x.values())
        ...
        cls = []
        for i in range(self.nl):
            dbox.append(self.cv2[i](x[i]))
            cls.append(self.cv3[i](x[i]))

        # Segmentation branch
        mask = [self.cv4[i](x[i]) for i in range(self.nl)]
        proto = self.cv5(x[0])  # prototype masks

        if self.training:
            return torch.cat([torch.cat((dbox[i], cls[i]), 1) for i in range(self.nl)], dim=1), \
                torch.cat(mask, dim=1), proto
        ...
        box = torch.cat([xi.view(shape[0], self.no, -1) for xi in dbox], 2)
        cls = torch.cat([xi.view(shape[0], self.nc, -1) for xi in cls], 2)
        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides

        # Process segmentation outputs
        mask = torch.cat([xi.view(shape[0], self.nm, -1) for xi in mask], 2)

        if self.export:
            # For export we return detection and segmentation outputs separately
            return (dbox, cls.sigmoid()), (mask.sigmoid(), proto)

        return (torch.cat((dbox, cls.sigmoid()), 1), (mask.sigmoid(), proto))

    def bias_init(self):
        """Initialize biases for detection and segmentation heads."""
        # Detection head bias init
        for a, b, s in zip(self.cv2, self.cv3, self.stride):
            a[-1].bias.data[:] = 1.0  # box
            b[-1].bias.data[:self.nc] = math.log(5 / self.nc / (640 / s) ** 2)  # cls

        # Segmentation head bias init
        for m in self.cv4:
            m[-1].bias.data[:self.nm] = math.log(0.1 / (self.nm / 2))  # mask
